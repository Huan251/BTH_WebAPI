...
import { createChiTietPhieuNhapSchema } from "../validators/chitiet_phieunhaps/create-chitiet_phieunhap.validator.js";
...
themChiTietPhieuNhap: async (req, res, next) => {
  try {
    const payload = createChiTietPhieuNhapSchema.parse(req.body);
    await chiTietPNService.create(payload);
    res.json({ message: "OK" });
  } catch (err) {
    next(err);
  }
